...
```bash
cd repo/uae-corporate-registry   # or whichever of the 4 actor repos you're setting up
APIFY_TOKEN=apify_api_xxxxxxxx ./docs/setup_github_remote.sh uae-corporate-registry-monitor
```
...
```bash
cd repo/uae-corporate-registry
gh repo create uae-corporate-registry-monitor --private --source=. --remote=origin
...
Repeat this for each of the other 3 repos (`repo/ted-procurement/`, `repo/uk-modern-slavery/`,
`repo/sovereign-debt/`) with a distinct repository name each — do not push more than one actor's
